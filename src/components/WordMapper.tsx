...
const WordMapper = (prop:props) => {
    ...
    const words = ["laihflauifalafdfa", "alfhfuiilafsdfs", "faiuliuhgregiegu", "aoifuhilulirggre", "aiufhregherhghg", "aiufiluhfaaiuf", "lafuihdlfualfas"];
    ...
    const litColors = (wordArr:string[]) => {
        console.log(wordArr)
   ...
   const helper:boolean  = prop.prop?.length === 0
    ...
    return (
        <div className="w-full flex flex-row gap-4 justify-around flex-wrap">
                            ...
                            words.map((word, idx) => {
                                return (
                                    <p key={idx} className={`cursor-pointer text-xl ${dispatchClass(idx)}`}>
                                        {word}
                                    </p>
                                )
                            })}
